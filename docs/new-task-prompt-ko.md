...
응답을 끝낼 때 현재 상태, 권장 다음 작업 하나, 행동 주체를 반드시 적어라.
사용자 테스트가 필요 없다는 말만 하고 끝내지 말고 남은 로드맵에서 다음 작업을
선택하라.
```
...
```text
[RPi-VMS M1-A] SUNAPI/Pi/Windows 시간 기준 및 offset 측정
[RPi-VMS M1-B] GStreamer ingest 지연 계측
...
- task 종료 시 다음 task 제목과 첫 메시지를 제시하며, 사용자가 지금 해야 할 일이
  없으면 그 사실과 다음 agent 작업을 함께 명시한다.
