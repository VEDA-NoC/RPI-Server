...
- PR #4를 `main`에 squash merge (`e163e824`)
- M1은 camera/Pi/Windows의 clock offset 측정과 결과 해석까지로 종료
- Qt/VMS live 경로가 준비되지 않았으므로 영상 end-to-end 측정은 후속 M9로 이동

## 바로 다음 작업

Milestone M2: 단일 채널 `ChannelIngest`

1. 프로세스 실행 시 RTSP 연결을 열고 client session과 독립적으로 유지
2. depay/parse 이후 recording/live 계통을 분리할 ownership 구조 확정
3. recording 경로는 frame을 drop하지 않고 live 경로만 latency 제한 정책 허용
4. 첫 buffer, reconnect, packet loss 상태를 기록

다음 task는 `[RPi-VMS M2] 단일 채널 ChannelIngest`다. 영상 end-to-end `200ms`
검증은 Qt viewer가 실제 경로를 재생할 수 있고 M3 live server가 준비된 뒤 M9에서
수행한다. M1 결과만으로 M9를 바로 시작하지 않는다.
