...
VMS source=~/rpi-vms
Hanwha camera=CAMERA_IP:554
Hanwha camera model=HANWHA_CAMERA_MODEL
camera firmware=CAMERA_FIRMWARE
camera firmware build=CAMERA_FIRMWARE_BUILD
camera Wisenet platform=CAMERA_PLATFORM_VERSION
camera CGI=2.6.6
camera ONVIF=24.06
camera OpenSDK=25.02.25
...
- GStreamer pipeline debug 로그에 camera credential이 포함되는 보안 문제가 있다.
