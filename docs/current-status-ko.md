...
1. **M1-A 실제 측정:** 구현한 read-only 도구를 실제 camera/Pi/Windows에서
   실행하고 결과 JSON을 검토
2. 측정 결과를 근거로 camera `SyncType` 변경 여부와 공통 NTP 기준 결정
...
바로 다음 task는 `[RPi-VMS M1-A] SUNAPI/Pi/Windows 시간 기준 및 offset 측정`이다.
먼저 상태 수집과 측정 도구를 구현하고, 실제 camera NTP 설정 변경은 수집 결과와
SUNAPI 문서를 검토한 뒤 별도로 승인받아 수행한다.
