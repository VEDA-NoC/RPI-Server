...
- camera `SyncType=Manual`, `camera - Pi = -2422.600ms`, SUNAPI 초 단위
  해상도를 포함한 추정 불확실성 `±536.358ms`
...
1. **M1-A read-only 측정 완료:** 15초 NTP 요청 간격으로 camera/Pi/Windows의
   동일 peer offset과 불확실성을 확인
2. **다음:** 측정 결과를 근거로 camera `SyncType` 변경 여부와 공통 NTP 기준 결정
...
바로 다음 task는 `[RPi-VMS M1-A] 공통 NTP 기준 및 camera SyncType 변경안 결정`이다.
15초 간격 재측정 결과와 SUNAPI 문서를 근거로 공통 NTP 기준과 변경·검증 절차를 먼저
확정한다. 실제 camera NTP 설정 변경은 별도로 승인받은 뒤 수행한다.
